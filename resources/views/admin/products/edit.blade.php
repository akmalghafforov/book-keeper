            ...
            <form action="{{ route('admin.products.update', $product) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="default_unit" id="default_unit"
                        ...
                        <option value="per_ton" {{ old('default_unit', $product->default_unit) === 'per_ton' ? 'selected' : '' }}>{{ __('per_ton') }}</option>
                        <option value="per_bag" {{ old('default_unit', $product->default_unit) === 'per_bag' ? 'selected' : '' }}>{{ __('per_bag') }}</option>
                        <option value="per_piece" {{ old('default_unit', $product->default_unit) === 'per_piece' ? 'selected' : '' }}>{{ __('per_piece') }}</option>
                    </select>
                    @error('default_unit')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    <label for="default_provider_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Default Provider (Optional)') }}</label>
                    <select name="default_provider_id" id="default_provider_id"
                        class="block w-full px-3 py-2 bg-white dark:bg-[#0a0a0a] border border-gray-300 dark:border-[#3E3E3A] text-gray-900 dark:text-white rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all duration-200">
                        <option value="">{{ __('None') }}</option>
                        @foreach($providers as $provider)
                            <option value="{{ $provider->id }}" {{ old('default_provider_id', $product->default_provider_id) == $provider->id ? 'selected' : '' }}>
                                {{ $provider->name }}
                            </option>
                        @endforeach
                    </select>
                    @error('default_provider_id')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>

                <div>
                    <label for="buy_price" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Buy Price (Optional)') }}</label>
                    <input type="number" step="0.0001" min="0" name="buy_price" id="buy_price" value="{{ old('buy_price', $product->buy_price) }}"
                        class="block w-full px-3 py-2 bg-white dark:bg-[#0a0a0a] border border-gray-300 dark:border-[#3E3E3A] text-gray-900 dark:text-white rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all duration-200"
                        placeholder="0.0000">
                    @error('buy_price')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
