...
@section('header_title', __('Add Product'))

@section('content')
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">{{ __('Add Product') }}</h2>
            ...
            <form action="{{ route('admin.products.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="product_category_id" id="product_category_id" required class="block w-full px-3 py-2 bg-white dark:bg-[#0a0a0a] border border-gray-300 dark:border-[#3E3E3A] text-gray-900 dark:text-white rounded-lg">
                        ...
                        @foreach($productCategories as $category)<option value="{{ $category->id }}" @selected(old('product_category_id') == $category->id)>{{ $category->name }}</option>@endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="default_provider_id" id="default_provider_id"
                        ...
                        @foreach($providers as $provider)
                            <option value="{{ $provider->id }}" {{ old('default_provider_id') == $provider->id ? 'selected' : '' }}>
                                {{ $provider->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" step="0.0001" min="0" name="buy_price" id="buy_price" value="{{ old('buy_price') }}"
                        class="block w-full px-3 py-2 bg-white dark:bg-[#0a0a0a] border border-gray-300 dark:border-[#3E3E3A] text-gray-900 dark:text-white rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all duration-200"
                        placeholder="0.0000">
                    @error('buy_price')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
