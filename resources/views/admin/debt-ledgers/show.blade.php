    ...
    <div class="bg-white dark:bg-[#161615] overflow-hidden shadow-sm sm:rounded-xl border border-gray-200 dark:border-[#3E3E3A]">
        <div class="p-8">
            <dl class="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
                ...
                @if($debtLedger->type === 'payment')
                    <div class="sm:col-span-1">
                        <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Payment Method') }}</dt>
                        <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $debtLedger->payment_method?->label() ?? __('N/A') }}</dd>
                    </div>
                @endif
                <div class="sm:col-span-1">
                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('Payment Purpose') }}</dt>
                    <dd class="mt-1 text-sm text-gray-900 dark:text-white">{{ $debtLedger->payment_purpose?->label($debtLedger->payer_name) ?? __('N/A') }}</dd>
                </div>
                <div class="sm:col-span-1">
