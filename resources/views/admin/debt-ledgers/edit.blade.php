...
@section('content')
<div class="max-w-2xl mx-auto space-y-6" x-data="{ clientId: '{{ old('client_id') ?? $debtLedger->client_id }}', type: '{{ old('type', $debtLedger->type) }}' }">
                ...
                <div x-show="type === 'payment'" x-cloak>
                    ...
                    <select name="payment_method" id="payment_method" :required="type === 'payment'"
                        ...
                        @foreach(\App\Enums\PaymentMethod::cases() as $paymentMethod)
                            <option value="{{ $paymentMethod->value }}" {{ old('payment_method', $debtLedger->payment_method?->value) === $paymentMethod->value ? 'selected' : '' }}>{{ $paymentMethod->label() }}</option>
                        @endforeach
                    </select>
