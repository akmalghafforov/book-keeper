            ...
            <form action="{{ route('admin.debt-ledgers.update', $debtLedger) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        ...
                        <option value="payment" {{ (old('type') ?? $debtLedger->type) == 'payment' ? 'selected' : '' }}>Payment</option>
                        <option value="credit_note" {{ (old('type') ?? $debtLedger->type) == 'credit_note' ? 'selected' : '' }}>Credit Note</option>
                    </select>
                    @error('type')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
