@if($potentialDuplicateGroups->isNotEmpty())
    <div class="bg-amber-50 dark:bg-[#261c08] border border-amber-200 dark:border-amber-900 rounded-xl p-6 space-y-4">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
                <h3 class="text-lg font-semibold text-amber-900 dark:text-amber-100">{{ __('Potential Duplicate Entries') }}</h3>
                <p class="mt-1 text-sm text-amber-800 dark:text-amber-200">
                    {{ __('These groups share the same client, type, amount, and transaction date. Review them before editing or deleting records.') }}
                </p>
            </div>
                ...
                <div class="rounded-xl border border-amber-200 dark:border-amber-900 bg-white dark:bg-[#161615] overflow-hidden">
                    <div class="px-5 py-4 border-b border-amber-100 dark:border-amber-950 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                        ...
                        <div class="flex flex-col items-start gap-3 text-sm font-semibold text-amber-900 dark:text-amber-100 lg:items-end">
                            ...
                            <form action="{{ route('admin.debt-ledgers.potential-duplicates.resolve') }}" method="POST">
                                @csrf
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-[#3E3E3A]">
                            <thead class="bg-gray-50 dark:bg-[#1C1C1A]">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Type') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Amount') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Date') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Reference') }}</th>
                                    <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">{{ __('Notes') }}</th>
                            ...
                            <tbody class="divide-y divide-gray-200 dark:divide-[#3E3E3A] bg-white dark:bg-[#161615]">
                                @foreach($group['records'] as $ledger)
                                    <tr>
                                        <td class="px-4 py-3 text-sm font-medium text-gray-900 dark:text-white">#{{ $ledger->id }}</td>
                                        ...
                                        <td class="px-4 py-3 text-right text-sm font-medium whitespace-nowrap">
                                            <a href="{{ route('admin.debt-ledgers.show', $ledger) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300">{{ __('View') }}</a>
                                            <span class="mx-1 text-gray-300 dark:text-gray-600">|</span>
                                            <a href="{{ route('admin.debt-ledgers.edit', $ledger) }}" class="text-yellow-600 hover:text-yellow-900 dark:text-yellow-400 dark:hover:text-yellow-300">{{ __('Edit') }}</a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
