...
@section('content')
<div class="max-w-2xl mx-auto space-y-6" x-data="{ providerId: '{{ old('provider_id', $selectedProviderId ?? '') }}', type: '{{ old('type', 'payment') }}', paymentPurpose: @js(old('payment_purpose', '')) }">
    ...
    <div class="bg-white dark:bg-[#161615] overflow-hidden shadow-sm sm:rounded-xl border border-gray-200 dark:border-[#3E3E3A]">
        <div class="p-8">
            <form action="{{ route('admin.provider-ledgers.store') }}" method="POST" class="space-y-6">
                @csrf
