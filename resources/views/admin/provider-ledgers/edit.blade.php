...
@section('title', __('Edit Provider Ledger Entry'))
@section('header_title', __('Edit Provider Ledger Entry'))
...
@section('content')
<div class="max-w-2xl mx-auto space-y-6" x-data="{ providerId: '{{ old('provider_id', $providerLedger->provider_id) }}' }">
    ...
    <div class="bg-white dark:bg-[#161615] overflow-hidden shadow-sm sm:rounded-xl border border-gray-200 dark:border-[#3E3E3A]">
        <div class="p-8">
            <form action="{{ route('admin.provider-ledgers.update', $providerLedger) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <div class="flex justify-between items-center mb-1">
                        <label for="provider_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ __('Provider') }}</label>
                    </div>
                    <select name="provider_id" id="provider_id" x-ref="select"
                        x-init="
                            $($refs.select).select2({
                            ...
                            $($refs.select).on('change', () => { providerId = $($refs.select).val() });
                        "
                        x-effect="$($refs.select).val(providerId).trigger('change')"
                        required
                        ...
                        @foreach($providers as $provider)
                            <option value="{{ $provider->id }}" {{ old('provider_id', $providerLedger->provider_id) == $provider->id ? 'selected' : '' }}>{{ $provider->name }}</option>
                        @endforeach
                    </select>
                    @error('provider_id')
                ...
                <div>
                    ...
                    <select name="type" id="type" required
                        class="block w-full px-3 py-2 bg-white dark:bg-[#0a0a0a] border border-gray-300 dark:border-[#3E3E3A] text-gray-900 dark:text-white rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all duration-200">
                        <option value="charge" {{ (old('type') ?? $providerLedger->type) === 'charge' ? 'selected' : '' }}>{{ __('charge') }}</option>
                        <option value="payment" {{ (old('type') ?? $providerLedger->type) === 'payment' ? 'selected' : '' }}>{{ __('payment') }}</option>
                    </select>
                    @error('type')
                ...
                <div>
                    ...
                    <input type="number" name="amount" id="amount" value="{{ old('amount', $providerLedger->amount) }}" step="0.01" min="0.01" required
                        class="block w-full px-3 py-2 bg-white dark:bg-[#0a0a0a] border border-gray-300 dark:border-[#3E3E3A] text-gray-900 dark:text-white rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all duration-200"
                        placeholder="0.00">
                    @error('amount')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" name="car_number" id="car_number" value="{{ old('car_number', $providerLedger->car_number) }}"
                        class="block w-full px-3 py-2 bg-white dark:bg-[#0a0a0a] border border-gray-300 dark:border-[#3E3E3A] text-gray-900 dark:text-white rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all duration-200">
                    @error('car_number')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div x-data="{
                    init() {
                        flatpickr($refs.datepicker, {
                            dateFormat: 'd/m/Y',
                            defaultDate: '{{ old('transaction_date', optional($providerLedger->transaction_date)?->format('d/m/Y')) }}',
                            allowInput: true,
                        });
                    }
                }">
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="3"
                        class="block w-full px-3 py-2 bg-white dark:bg-[#0a0a0a] border border-gray-300 dark:border-[#3E3E3A] text-gray-900 dark:text-white rounded-lg shadow-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm transition-all duration-200">{{ old('notes', $providerLedger->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-2 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
